        ...
        if (context.plugin.server.config.user.dataserviceAuthentication.rbac) {
            if (req.body.recipient === INDIVIDUAL) {
                ...
                if (index != -1) {
                    ...
                    if (sent) {
                        res.status(201).json({"Response" : "ZWED5000I - Message sent to " + req.body.recipient});
                    } else {
                        res.status(500).json({"Response" : "ZWED5008E - Server error"});
                    }
                } else {
                    if (req.body.username === "") {
                        res.status(404).json({"Response" : "ZWED5002E - Recipient input cannot be blank"});
                    } else {
                        res.status(404).json({"Response" : "ZWED5003E - " + req.body.username + " is not a valid user or is not online"});
                    }
                }
                ...
                if (sent) {
                    res.status(201).json({"Response" : "ZWED5002I - Message sent to " + req.body.recipient});
                } else {
                    res.status(500).json({"Response" : "ZWED5004E - Server error"});
                }
            } else {
                res.status(400).json({"Response": "ZWED5005E - Message was not sent"})
            }
        } else {
            res.status(403).json({"Response": "ZWED5006E - RBAC is not enabled"})
        }
      });
