...
export class Viewport {
  ...
  checkProviders(): void {
    if (!this.providers.has(Angular2InjectionTokens.VIEWPORT_EVENTS)) {
      this.logger.warn('ZWED0165W'); //this.logger.warn('No VIEWPORT_EVENTS provided to custom viewport. This may cause plugins to behave unexpectedly.');
    }

    if (!this.providers.has(Angular2InjectionTokens.PLUGIN_EMBED_ACTIONS)) {
      this.logger.warn('ZWED0166W'); //this.logger.warn('No PLUGIN_EMBED_ACTIONS provided to custom viewport. This may cause plugins to behave unexpectedly.');
    }
  }
}
