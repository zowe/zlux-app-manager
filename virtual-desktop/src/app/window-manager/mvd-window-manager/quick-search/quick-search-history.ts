...
/**
 * Command-history persistence for quick search (TSO / MVS command history via
 * the Zowe config dataservice).
 *
 * IMPORTANT: this is a PLAIN class, instantiated via `new` by QuickSearchService
 * and NOT an Angular @Injectable / module provider. This is deliberate. Adding
 * extra providers to WindowManagerModule reshapes the desktop's AOT bundle and
 * trips an esbuild module-init threshold that makes the runtime fall back to the
 * (absent) JIT compiler, breaking ALL plugin loading. Composing plain helper
 * classes (the same pattern used by CompiledPlugin / Viewport / ApplicationInstance)
 * keeps the code split without touching the module provider graph.
 */
export class QuickSearchHistory {
