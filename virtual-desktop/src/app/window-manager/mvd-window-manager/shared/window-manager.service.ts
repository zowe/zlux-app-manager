...
export class WindowManagerService implements MVDWindowManagement.WindowManagerServiceInterface {
  ...
  private generateNewWindowPosition(plugin: DesktopPluginDefinitionImpl): WindowPosition {
    ...
    if (nextLeft > rightMostPosition) {
      ...
      this.logger.debug("ZWED0318I", dtWindowWidth, newWidth, nextLeft, innerWidth); //this.logger.debug(`reducing width from ${dtWindowWidth} to ${newWidth} based on nextLeft ${nextLeft} and innerWidth ${innerWidth}`);
      dtWindowWidth = newWidth;
    }

    if (nextTop > bottomMostPosition) {
      const newHeight = innerHeight - nextTop;
      this.logger.debug("ZWED0319I", dtWindowHeight, newHeight, nextTop, innerHeight); //this.logger.debug(`reducing height from ${dtWindowHeight} to ${newHeight} based on nextTop ${nextTop} and innerHeight ${innerHeight}`);
    ...
    return {
      createEmbeddedInstance: (identifier: string, launchMetadata: any, viewContainer: ViewContainerRef): Promise<EmbeddedInstance> => {
        return this.pluginManager.findPluginDefinition(identifier).then((plugin): InstanceId => {
          if (plugin == null) {
            throw new Error('ZWED0018E - No matching plugin definition found');
          }
  ...
  getViewportId(windowId: MVDWindowManagement.WindowId): MVDHosting.ViewportId {
    ...
    if (desktopWindow == null) {
      throw new Error('ZWED0019E - Attempted to retrieve viewport id of null window');
    }
  ...
  closeWindow(windowId: MVDWindowManagement.WindowId): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      this.logger.warn("ZWED0181W", windowId); //this.logger.warn(`Attempted to close null window, ID=${windowId}`);
     return;
    }
    ...
    const closeViewports = ()=> {
      desktopWindow.closeViewports(this.viewportManager).then(()=> {
        ...
        this.logger.warn("ZWED0182W", info); //this.logger.warn(`Window could not be closed because of viewport. Details=`,info);
        return;
      }); 
    }
  ...
  registerCloseHandler(windowId: MVDWindowManagement.WindowId, handler: () => Promise<void>): void {
    this.logger.warn("ZWED0183W"); //this.logger.warn(`windowActions.registerCloseHandler is deprecated. Please use viewportEvents.registerCloseHandler instead.`);
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      this.logger.warn("ZWED0184W", windowId); //this.logger.warn('Attempted to register close handler for null window, ID=${windowId}');
      return;
    }
  ...
  getWindowTitle(windowId: MVDWindowManagement.WindowId): string | null {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      this.logger.warn("ZWED0185W", windowId); //this.logger.warn('Attempted to set window title for null window, ID=${windowId}');
      return null;
    }
  ...
  setWindowTitle(windowId: MVDWindowManagement.WindowId, title: string): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      this.logger.warn("ZWED0186W", windowId); //this.logger.warn('Attempted to set window title for null window, ID=${windowId}');
      return;
    }
  ...
  requestWindowFocus(destination: MVDWindowManagement.WindowId): boolean {
    ...
    if (desktopWindow == null) {
      this.logger.warn("ZWED0187W", destination); //this.logger.warn('Attempted to request focus for null window, ID=${destination}');
      return false;
    }
  ...
  getWindowEvents(windowId: MVDWindowManagement.WindowId): LocalWindowEvents {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0020E - Attempted to get window events for null window');
    }
  ...
  maximize(windowId: MVDWindowManagement.WindowId): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0021E - Attempted to maximize null window');
    }
  ...
  minimize(windowId: MVDWindowManagement.WindowId): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0022E - Attempted to minimize null window');
    }
  ...
  restore(windowId: MVDWindowManagement.WindowId): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0023E - Attempted to restore null window');
    }
  ...
  setPosition(windowId: MVDWindowManagement.WindowId, pos: WindowPosition): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0024E - Attempted to set position for null window');
    }
  ...
  maximizeToggle(windowId: MVDWindowManagement.WindowId): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0025E - Attempted to maximize toggle null window');
    }
  ...
  minimizeToggle(windowId: MVDWindowManagement.WindowId): void {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0026E - Attempted to minimize toggle null window');
    }
  ...
  spawnContextMenu(windowId: MVDWindowManagement.WindowId, x: number, y: number, items: ContextMenuItem[], isAbsolutePos?: boolean): boolean {
    const desktopWindow = this.windowMap.get(windowId);
    if (desktopWindow == null) {
      throw new Error('ZWED0027E - Attempted to spawn context menu for null window');
    }
