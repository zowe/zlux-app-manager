...
export class PersonalizationComponent implements AfterViewInit {
  ...
  clickSlider($event: any) {
    const index = ($event.offsetY * this.sliderImgData.width + $event.offsetX) * 4;
    const red = this.sliderImgData.data[index];
    const green = this.sliderImgData.data[index + 1];
    const blue = this.sliderImgData.data[index + 2];
    ...
    const textColor = Colors.COOLGREY_10;
