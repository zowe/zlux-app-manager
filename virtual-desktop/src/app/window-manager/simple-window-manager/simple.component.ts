...
export class SimpleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!requestedPluginID) {
      ...
      this.logger.severe("ZWED0143E"); //this.logger.severe(message);
      this.error = message;
      return;
    }
    ...
    pluginPromise.then(plugin => {
      if (plugin) {
        ...
        this.logger.severe("ZWED0144E", pluginID); //this.logger.severe(message);
        this.error = message;
      }
    }).catch((x) => {
      let message = "Plugin promise not returned, cannot continue";
      this.logger.severe("ZWED0145E"); //this.logger.severe(message);
      this.error = message;
      return;
    });
  }
