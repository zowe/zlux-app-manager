...
export class LoginComponent implements OnInit {
  ...
  constructor(
    ...
    this.authenticationService.loginScreenVisibilityChanged.subscribe((eventReason: LoginScreenChangeReason) => {
      ...
      case LoginScreenChangeReason.SessionExpired:
        ...
        break;
      default:
        this.logger.warn('ZWED0168W', eventReason); //this.logger.warn('Ignoring unknown login screen change reason='+eventReason);
      }
    ...
    this.authenticationService.loginExpirationIdleCheck.subscribe((e: LoginExpirationIdleCheckEvent)=> {
      ...
      if (!this.isIdle()) {
        this.logger.info('ZWED0007I'); /*this.logger.info('Near session expiration, but renewing session due to activity');*/
        this.renewSession();
      } else {
        this.logger.info('ZWED0008I'); /*this.logger.info('Near session expiration. No activity detected, prompting to renew session');*/
  ...
  private isIdle(): boolean {
    ...
    this.logger.debug("ZWED0304I", this.lastActive, Date.now(), idle); //this.logger.debug(`User lastActive=${this.lastActive}, now=${Date.now()}, idle={idle}`);
    return idle;
  }
  ...
  detectActivity(): void {
    this.logger.debug('ZWED0305I'); //this.logger.debug('User activity detected');
